...
export default function WildartDetailContent({ slug }: { slug: string }) {
      ...
      <div className="flex flex-col min-h-full">
        <div className="relative px-[22px] pb-[26px] text-[#EAF3EC] overflow-hidden"
          ...
          <a
            ...
            onClick={goBack}
            onTouchEnd={goBack}
            className="absolute left-[14px] w-[38px] h-[38px] rounded-full grid place-items-center z-[3] text-[#EAF3EC] no-underline active:scale-[.92] border-0 p-0 cursor-pointer"
            style={{ background: "rgba(255,255,255,.14)", top: "calc(14px + env(safe-area-inset-top, 0px))" }}
            aria-label="Zurück"
          >
            <ArrowLeft size={18} />
          </a>
  ...
  return (
    <div className="flex flex-col min-h-full">
      {/* Hero */}
      <div className="relative px-[22px] pb-[26px] text-[#EAF3EC] overflow-hidden" style={{ background: toneGradient, paddingTop: "calc(56px + env(safe-area-inset-top, 0px))" }}>
        <a
          href="/"
          onClick={goBack}
          onTouchEnd={goBack}
